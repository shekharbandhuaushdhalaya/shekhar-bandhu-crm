...
function PipelineColumn({ stage, contacts, onMoveContact, width }: { stage: Stage; contacts: Contact[]; onMoveContact: (id: string, newStage: string) => void; width: number }) {
  ...
  return (
    <View style={[styles.column, { borderTopColor: stageColor, width }]}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        {contacts.map(c => (
          <View key={c._id} style={styles.dealCard}>
        ...
        {contacts.length === 0 && (
          <View style={styles.emptyColumn}>
            ...
            <Text style={styles.emptyColText}>No deals</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
export default function PipelineScreen() {
  ...
  const totalPipeline = contacts.filter(c => c.stage !== 'won' && c.stage !== 'lost').reduce((s, c) => s + c.dealValue, 0);
  const wonTotal = contacts.filter(c => c.stage === 'won').reduce((s, c) => s + c.dealValue, 0);
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView horizontal pagingEnabled={false} showsHorizontalScrollIndicator={winWidth <= 1100} style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: Spacing.md, gap: Spacing.md, paddingBottom: Spacing.lg }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary} />}>
        {STAGES.map(stage => (
          <PipelineColumn key={stage} stage={stage} contacts={contacts.filter(c => c.stage === stage)} onMoveContact={handleMove} width={columnWidth} />
        ))}
      </ScrollView>
    </View>
  );
