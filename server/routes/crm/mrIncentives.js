...
router.get('/calculate', authorize('mr:view'), async (req, res) => {
  try {
    ...
    for (const mr of mrs) {
      const calc = await calculateMrIncentive(mr._id, targetYear, targetMonth);
      ...
      results.push({
        ...
        actualAchievedAmount: calc.actualAchievedAmount,
        achievementPercent: calc.achievementPercent,
        slabCommissionPercent: calc.slabCommissionPercent,
        incentivePayoutAmount: calc.incentivePayoutAmount
      });
    }
