...
async function computeDemandForecast() {
  ...
  return products.map(p => {
    ...
    const projectedNextMonthUnits = Math.ceil(Number((avgMonthlyDemand * 1.1).toFixed(6))); // 10% seasonal growth buffer
...
router.get('/', authorize('report:view'), async (req, res) => {
  try {
    const forecastResults = await computeDemandForecast();
...
module.exports.computeDemandForecast = computeDemandForecast;
