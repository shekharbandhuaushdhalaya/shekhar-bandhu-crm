...
const invoiceItem = z.object({
  productId: objectId.optional(),
  rawMaterialId: objectId.optional(),
  itemType: z.enum(['product', 'raw_material', 'packaging', 'consumable', 'excipient', 'semi_finished', 'other']).optional(),
  name: z.string().min(1),
...
const batchCompleteSchema = z.object({
  ...
  moistureLimit: z.string().optional(),
  ashValue: z.number().nullable().optional(),
  ashValueLimit: z.string().optional(),
  pHValue: z.number().nullable().optional(),
  pHLimit: z.string().optional(),
  disintegrationTime: z.number().nullable().optional(),
  disintegrationLimit: z.string().optional(),
...
const challanSchema = z.object({
  ...
  sourceManufacturingUnitId: objectId.nullable().optional(),
  sourceManufacturingUnitName: z.string().default(''),
...
const rawMaterialSchema = z.object({
  materialType: z.enum(['raw_material', 'packaging', 'consumable', 'excipient', 'semi_finished', 'other']).optional().default('raw_material'),
  packagingType: z.enum(['bottle', 'cap', 'label', 'carton', 'corrugated_box', 'pouch', 'sachet', 'foil', 'shrink_wrap', 'insert', 'tape', 'other', '']).optional().default(''),
  ...
  botanicalName: z.string().optional().default(''),
  acceptedScientificName: z.string().optional().default(''),
  family: z.string().optional().default(''),
  genus: z.string().optional().default(''),
  species: z.string().optional().default(''),
  botanicalAuthority: z.string().optional().default(''),
  taxonomicRank: z.string().optional().default(''),
  taxonomicStatus: z.string().optional().default(''),
  botanicalSynonyms: z.array(z.string()).optional().default([]),
  commonNames: z.array(z.string()).optional().default([]),
  taxonomySource: z.string().optional().default(''),
  taxonomyVerifiedAt: z.string().or(z.date()).optional().nullable(),
  therapeuticUses: z.array(z.string()).optional().default([]),
  rasa: z.array(z.string()).optional().default([]),
  virya: z.string().optional().default(''),
  vipaka: z.string().optional().default(''),
  guna: z.array(z.string()).optional().default([]),
  dosage: z.string().optional().default(''),
  botanicalDescription: z.string().optional().default(''),
  partUsed: z.string().optional().default(''),
  pharmacopoeialStandard: z.enum(['API', 'AFI', 'IP', 'BP', 'USP', 'House Standard']).optional().default('API'),
  monographRef: z.string().optional().default(''),
});
...
const bomSchema = z.object({
  ...
  formulationStandardRef: z.string().optional().default(''),
...
const medicalRepSchema = z.object({
  ...
  territories: z.array(z.string().trim()).optional().default([]),
  division: z.string().trim().optional().default(''),
  team: z.string().trim().optional().default(''),
  designation: z.string().trim().optional().default('Medical Representative'),
  employmentType: z.enum(['full_time', 'part_time', 'contract', 'other']).optional().default('full_time'),
  backupMrId: objectId.optional().nullable(),
  defaultVisitTypes: z.array(z.string().trim()).optional().default([]),
  targetConfig: z.object({
    sales: z.coerce.number().finite().min(0).optional().default(0),
    visits: z.coerce.number().finite().min(0).optional().default(0),
    newDoctors: z.coerce.number().finite().min(0).optional().default(0),
    followUps: z.coerce.number().finite().min(0).optional().default(0),
    orders: z.coerce.number().finite().min(0).optional().default(0),
  }).optional(),
...
const mrVisitSchema = z.object({
  ...
  location: z.string().default(''),
  notes: z.string().default(''),
  purpose: z.enum(['promotion','sampling','collection','followup','meeting','chemist','stockist','institution','other']).optional().default('promotion'),
  visitType: z.enum(['doctor','chemist','stockist','hospital','distributor','followup','collection','other']).optional().default('doctor'),
  outcome: z.enum(['interested','follow_up','prescribing','order','not_interested','competitor','no_meeting','other']).optional().default('other'),
  followUpAt: z.string().or(z.date()).nullable().optional(),
  jointWithMrId: objectId.optional().nullable(),
  jointWithName: z.string().optional().default(''),
  promotedProducts: z.array(z.object({ productId: objectId.optional(), name: z.string().optional().default(''), response: z.string().optional().default('') })).optional().default([]),
  date: z.string().or(z.date()).optional(),
  latitude: z.number().optional(),
  longitude: z.number().optional(),
